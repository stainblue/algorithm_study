...
## 목차
1. [LIS 란?](#1-lis-란)
2. [LIS의 길이를 구하는 알고리즘](#2-lis의-길이를-구하는-알고리즘)   
	2-1. [완전탐색을 이용한 풀이 (O(2<sup>N</sup>))](#2-1-완전탐색을-이용한-풀이)   
	2-2. [DP를 이용한 풀이 (O(N<sup>2</sup>))](#2-2-dp를-이용한-풀이)   
	2-3. [이분탐색을 이용한 풀이 (O(NlogN))](#2-3-이분탐색을-이용한-풀이)   
